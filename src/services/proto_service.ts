...
class ProtobufService {
  ...
  private packageName: string = "pba"; // 默认 package 名称
  ...
  public encode(cmd: number, payload: object): ArrayBuffer {
    ...
    const Message = this.root.lookupType(`${this.packageName}.m_${cmd}_tos`);
  ...
  public decode(payload: ArrayBuffer): {
    ...
    if (reply.errorCode === 0) {
      ...
      const Message = this.root.lookupType(
        `${this.packageName}.m_${reply.cmd}_toc`
      );
  ...
  private unpack(bin: ArrayBuffer): {
    cmd: number;
    errorCode: number;
    msg?: Uint8Array;
  } {
